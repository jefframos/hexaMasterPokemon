var WandoScreen = AbstractScreen.extend({
    ...
    build: function () {
        ...
        this.levels;
        this.id = Math.floor(Math.random() * 486345654);
        ...
        var linkk = $.trim(boardConfig[5].value); //'https://jefframos.firebaseio.com/' + Math.floor(Math.random() * 98613146497865);
        console.log(linkk);
        myDataRef = new Firebase(linkk);
        myDataRef.push({
            type: "connect",
            id: this.id
        });
        ...
        myDataRef.on('child_added', function (snapshot) {
            ...
            if (message.id != that.id) {
                ...
                if (message.type == "build") {
                    //loadByOpponn message.builderGameInfo);
                    ...
                    for (var i = 0; i < message.builderGameInfo.length; i++) {
                        that.loadJSON(message.builderGameInfo[i].id, message.builderGameInfo[i].last, message.builderGameInfo[i].team, true);
                    };

                }else if(message.type == "putCard"){
                	console.log(message)
                	that.selectedCard =  that.cardsList[message.selectedCard];
    ...
    loadJSON: function (id, last, team, opponent) {
        var that = this;
        ...
        $.getJSON("assets/pokemon/json/pokedex_full_" + idStr + ".json", function (response) {
            that.jsonLoaded(response, idStr, last, team);
        });

    },
    //carrega pelo json
    jsonLoaded: function (response, idStr, last, team) {
        ...
        that.cardModelList.push(new CardModel(
            ...
            team,
            "assets/pokemon/" + response.thumb_url));
    ...
    clickOnGrid: function (grid) {
        ...
        if (this.selectedCard && this.getCardOnGrid(grid.id.i, grid.id.j) == null) {
        	...
        	myDataRef.push({type: "putCard", selectedCard: this.selectedCard.id, grid: grid.id, id:this.id});
	...
	**/
    ...
    createBoard: function (isHex, size, centerPoint, tileSize) {
        ...
        var createTile = function (i, j, position) {
            var tempTile = new GridTile("assets/img/card.png", i, j, position, that.gridList, that);
